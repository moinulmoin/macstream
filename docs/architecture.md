...
- `StudioStore.canStartStream`, `canStopStream`, and `canEditDestination` lock out duplicate starts, duplicate stops, and destination edits while live, stopping, or while the async connection step is in progress, including stale binding writes that try to mutate the destination model directly. Pipelines that perform real macOS capture, such as `SystemMediaPipeline`, also require a live scene instead of the initial BRB fallback, capture readiness, and at least one enabled source before stream or recording startup; preview/test pipelines can opt out so local logic tests do not pretend to need TCC. The current system media path records raw ScreenCaptureKit video for Screen and composed screen + camera video for Screen + Webcam; the HaishinKit publish path supports the same Screen and composed Screen + Webcam scenes. Webcam and BRB remain blocked for full RTMP until they have a non-screen media output. Endpoint-validation streams still validate RTMP reachability without claiming to publish media. Stream startup and recording startup also suppress starting the other capture path until the pending setup finishes, avoiding parallel ScreenCaptureKit startup spikes. Cancelling a connection invalidates that start attempt, and late completions are ignored and cleaned up.
...
- The main studio window uses a full preview column plus one collapsible,
  ...
  screen, and Screen + Webcam requires both for preview readiness, local
  composited recording, and HaishinKit full RTMP publishing; Webcam and BRB remain
  ...
  computed from the selected scene and enabled sources, so Webcam mode does not
