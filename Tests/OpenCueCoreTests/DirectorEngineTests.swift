...
func inspectorKeepsPrimaryControlsInSidePanel() throws {
    ...
    let navigationURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/StudioNavigationPanelView.swift")
    ...
    let navigationIndex = try #require(studioSource.range(of: "StudioNavigationPanelView(store: store)")?.lowerBound)
    ...
    #expect(navigationIndex < controlIndex)
    ...
    #expect(navigationSource.contains("struct StudioNavigationPanelView"))
    #expect(navigationSource.contains("Label(\"Scenes\""))
    #expect(navigationSource.contains("ForEach(store.scenes)"))
    #expect(navigationSource.contains("store.sceneSelectionBlockedReason(for: scene)"))
    #expect(navigationSource.contains("store.canSelectScene(scene)"))
    #expect(navigationSource.contains(".disabled(!canSelectScene)"))
    #expect(navigationSource.contains(".help(blocker ?? scene.subtitle)"))
    #expect(navigationSource.contains("store.selectScene(scene)"))
    #expect(navigationSource.contains("SceneChoiceRow("))
    #expect(!navigationSource.contains("store.setupRole(for: source.kind)"))
    #expect(!navigationSource.contains("SourceStatusRow("))
    #expect(!navigationSource.contains("store.startStream()"))
    #expect(!navigationSource.contains("store.startRecording()"))
    #expect(!navigationSource.contains("CapturePermissionActions.requestAccess"))
