...
struct OpenCueApp: App {
    ...
    private func applySavedDestination() {
        ...
        let rtmpURL = OpenCueDestinationKeychain.loadRTMPURL() ?? (mode == .rtmp ? "" : "preview")
    ...
    private func saveDestination(_ destination: StreamDestination) {
        ...
        if destination.usesPreviewSentinelURL || destination.rtmpURL.isEmpty {
            if !OpenCueDestinationKeychain.deleteRTMPURL() {
                store.reportPersistenceFailure("RTMP destination could not be removed from Keychain.")
            }
        } else {
            if !OpenCueDestinationKeychain.saveRTMPURL(destination.rtmpURL) {
