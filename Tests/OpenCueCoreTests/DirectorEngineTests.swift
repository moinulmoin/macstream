...
func packageMetadataDefinesRequiredCapturePrivacyKeys() throws {
    ...
    #expect(packageScript.contains("Resources/AppIcon/OpenCue.icns"))
    #expect(packageScript.contains("cp \"$APP_ICON\" \"$APP_RESOURCES/OpenCue.icns\""))
}
...
func packagingScriptsSignAppBundleWithStableIdentifier() throws {
    ...
    #expect(runScript.contains("BUNDLE_ID=\"com.ideaplexa.opencue\""))
    #expect(!runScript.contains("BUNDLE_ID=\"com.opencue.app\""))
    #expect(runScript.contains("Developer ID Application: Ideaplexa LLC (53P98M92V7)"))
    #expect(runScript.contains("OPEN_CUE_CODESIGN_IDENTITY"))
    #expect(runScript.contains("OPEN_CUE_CODESIGN_TIMESTAMP=\"${OPEN_CUE_CODESIGN_TIMESTAMP:-none}\""))
    #expect(runScript.contains("\"$ROOT_DIR/script/package_macos_app.sh\""))
    #expect(packageScript.contains("BUNDLE_ID=\"${OPEN_CUE_BUNDLE_ID:-com.ideaplexa.opencue}\""))
    #expect(packageScript.contains("OPEN_CUE_CODESIGN_IDENTITY"))
...
func screenCapturePermissionHintExplainsRestartRequirement() {
    ...
    #expect(display.permissionRecoveryHint == "Enable Screen Recording in System Settings. If it is already on, quit and reopen OpenCue.")
    #expect(window.permissionRecoveryHint == "Enable Screen Recording in System Settings. If it is already on, quit and reopen OpenCue.")
...
func screenCaptureScanDoesNotLoadShareableContentBeforePermissionIsVisible() async {
    ...
    #expect(screenDevice?.permissionRecoveryHint == "Enable Screen Recording in System Settings. If it is already on, quit and reopen OpenCue.")
...
func capturePreflightViewOffersRelaunchForRestartScopedPermissions() throws {
    let viewURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/CapturePreflightView.swift")
    let relauncherURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Support/OpenCueRelauncher.swift")
...
func inspectorKeepsPrimaryControlsInSidePanel() throws {
    ...
    let settingsURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/SettingsView.swift")
    let checklistURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/SetupChecklistView.swift")
    let sidebarURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/SidebarView.swift")
    let destinationURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/DestinationView.swift")
    let sourceRackURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/SourceRackView.swift")
    let storeURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCueCore/Stores/StudioStore.swift")
    ...
    #expect(studioSource.contains("@SceneStorage(\"OpenCue.StudioView.isInspectorCollapsed\")"))
    #expect(studioSource.contains("InspectorRailView(store: store, isInspectorCollapsed: $isInspectorCollapsed)"))
    #expect(studioSource.contains(".frame(width: 54)"))
    #expect(studioSource.contains(".frame(width: 340)"))
    ...
    #expect(studioSource.contains(".id(InspectorPanelID.studioControl)"))
    #expect(studioSource.contains(".onChange(of: store.shouldShowSetupChecklist)"))
    #expect(studioSource.contains("scrollProxy.scrollTo(InspectorPanelID.studioControl, anchor: .top)"))
    ...
    #expect(studioSource.contains("LazyVStack(alignment: .leading, spacing: 18)"))
    ...
    #expect(studioSource.contains("isInspectorCollapsed = false"))
    #expect(studioSource.contains(".help(\"Show controls\")"))
    ...
    #expect(controlSource.contains("performanceStatusTitle"))
    ...
    #expect(controlSource.contains("var onCollapse: (() -> Void)? = nil"))
    ...
    #expect(!controlSource.contains("OpenCueRelauncher.relaunch()"))
    #expect(!controlSource.contains("Label(\"Reopen OpenCue\""))
    #expect(controlSource.contains("if store.recordingState == .recording { return \"Recording\" }"))
    #expect(controlSource.contains("if store.shouldShowSetupChecklist { return \"Setup\" }"))
    ...
    #expect(controlSource.contains("return \"Stop Recording\""))
    #expect(controlSource.contains("return \"Cancel Recording\""))
    #expect(controlSource.contains(".minimumScaleFactor(0.82)"))
    ...
    #expect(appSource.contains("Window(\"OpenCue\", id: \"studio\")"))
    ...
    #expect(appSource.contains("OpenCueDestinationKeychain.loadRTMPURL()"))
    ...
    #expect(capturePreflightSource.contains("Label(\"Reopen OpenCue\""))
    #expect(capturePreflightSource.contains("@SceneStorage(\"OpenCue.CapturePreflightView.showDeviceDetails\")"))
    ...
    #expect(checklistSource.contains("Get Started"))
    ...
    #expect(checklistSource.contains("OpenCueRelauncher.relaunch()"))
    ...
    #expect(sidebarSource.contains("Section(\"Setup\")"))
    #expect(sidebarSource.contains("SidebarSetupStatusRow"))
    #expect(sidebarSource.contains("store.completedSetupItemCount"))
    #expect(sidebarSource.contains("store.totalSetupItemCount"))
    #expect(sidebarSource.contains("store.setupProgressFraction"))
    #expect(sidebarSource.contains("store.nextSetupChecklistItem"))
    #expect(sidebarSource.contains("store.sceneSelectionBlockedReason(for: scene)"))
    #expect(sidebarSource.contains("store.canSelectScene(scene)"))
    #expect(sidebarSource.contains(".disabled(!canSelectScene)"))
    #expect(sidebarSource.contains(".help(sceneSelectionBlocker ?? scene.subtitle)"))
    #expect(sidebarSource.contains("store.setupRole(for: source.kind)"))
    #expect(sidebarSource.contains("SourceStatusRow("))
    #expect(sidebarSource.contains("role.title"))
    #expect(sidebarSource.contains("sourceStateTitle"))
    #expect(sidebarSource.contains("return \"Muted\""))
    #expect(sidebarSource.contains("source.kind.supportsLevelControl && source.level <= 0"))
    #expect(sidebarSource.contains("case .required, .recommended:"))
    #expect(!sidebarSource.contains("store.startStream()"))
    #expect(!sidebarSource.contains("store.startRecording()"))
    #expect(!sidebarSource.contains("CapturePermissionActions.requestAccess"))
    ...
    #expect(sourceRackSource.contains("@SceneStorage(\"OpenCue.SourceRackView.showMoreSources\")"))
...
func screenPreviewDoesNotRequestScreenRecordingPermissionPassively() throws {
    let previewURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/NativePreview/ScreenCapturePreviewView.swift")
    let canvasURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/PreviewCanvasView.swift")
    let studioURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/StudioView.swift")
...
func signalSamplingDoesNotRequestScreenRecordingPermissionPassively() throws {
    let signalURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCueCore/Services/SignalProvider.swift")
...
func cameraPreviewDoesNotRequestCameraPermissionPassively() throws {
    let previewURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/NativePreview/CameraPreviewView.swift")
    let canvasURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/PreviewCanvasView.swift")
    let studioURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Views/StudioView.swift")
    ...
    let permissionActionsURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Support/CapturePermissionActions.swift")
    let previewSource = try String(contentsOf: previewURL, encoding: .utf8)
    let canvasSource = try String(contentsOf: canvasURL, encoding: .utf8)
    let studioSource = try String(contentsOf: studioURL, encoding: .utf8)
...
func packageManifestKeepsHeavyAIAndRTMPDependenciesOptIn() throws {
    ...
    #expect(manifest.contains("OPEN_CUE_ENABLE_HAISHINKIT"))
    #expect(manifest.contains("OPEN_CUE_ENABLE_MLX"))
...
func releaseAutomationDefinesSignedNotarizedMacPipeline() throws {
    ...
    let entitlementsURL = root.appendingPathComponent("Resources/Entitlements/OpenCue.Release.entitlements")
    let infoPlistURL = root.appendingPathComponent("Resources/Info.plist")
    let sourceIconURL = root.appendingPathComponent("Resources/AppIcon/OpenCue-AppIcon-Source.png")
    let iconURL = root.appendingPathComponent("Resources/AppIcon/OpenCue.icns")
    ...
    #expect(iconReadme.contains("OpenCue-AppIcon-Source.png"))
    ...
    #expect(ci.contains("OPEN_CUE_ENABLE_HAISHINKIT=1 swift build -c release --arch arm64"))
    #expect(ci.contains("OPEN_CUE_ENABLE_MLX=1 swift build -c release --arch arm64"))
    #expect(ci.contains("OPEN_CUE_REQUIRE_HARDENED_RUNTIME: \"1\""))
    ...
    #expect(release.contains("OPEN_CUE_MACOS_CERTIFICATE_P12_BASE64"))
    #expect(release.contains("OPEN_CUE_MACOS_CERTIFICATE_PASSWORD"))
    #expect(release.contains("OPEN_CUE_CODESIGN_IDENTITY"))
    #expect(release.contains("OPEN_CUE_APPLE_ID"))
    #expect(release.contains("OPEN_CUE_APPLE_TEAM_ID"))
    #expect(release.contains("OPEN_CUE_APP_SPECIFIC_PASSWORD"))
    #expect(release.contains("security import \"$certificate_path\""))
    #expect(release.contains("OPEN_CUE_REQUIRE_DEVELOPER_ID: \"1\""))
    #expect(release.contains("OPEN_CUE_REQUIRE_HARDENED_RUNTIME: \"1\""))
    ...
    #expect(docs.contains("OpenCue does not ship Sparkle or another in-app updater yet."))
}
...
func keychainPersistenceReportsFailuresToApp() throws {
    let keychainURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/Support/OpenCueDestinationKeychain.swift")
    let appURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("Sources/OpenCue/App/OpenCueApp.swift")
    ...
    #expect(appSource.contains("if !OpenCueDestinationKeychain.saveRTMPURL(destination.rtmpURL)"))
    #expect(appSource.contains("if !OpenCueDestinationKeychain.deleteRTMPURL()"))
    #expect(appSource.contains("store.reportPersistenceFailure"))
}
...
func systemMediaPipelineOnlyCapturesPublishMediaForFullRTMPTransport() {
    ...
    #expect(pipeline.supportedSceneKindsForStream == [.screenOnly])
    #else
    #expect(!SystemMediaPipeline.capturesMediaForStreamTransport)
    #expect(!pipeline.requiresScreenCaptureVideoForStream)
...
func systemPressureUsesMemoryFootprintForEfficiency() {
    ...
    #expect(largeFootprint.efficiencyPressureDetail == "OpenCue is using 2048 MB; Efficiency mode is safer.")
    #expect(highShare.memoryUsagePercent == 25)
    #expect(highShare.isMemoryConstrained)
    #expect(highShare.efficiencyPressureDetail == "OpenCue is using 1024 MB; Efficiency mode is safer.")
}
...
func studioStoreWarnsAboutMemoryPressureWhenLive() async {
    ...
    #expect(store.events.contains {
        $0.title == "Performance pressure" && $0.detail == "OpenCue is using 2048 MB; Efficiency mode is safer."
    })
}
...
func captureReadinessTreatsScreenRecordingAsRestartScoped() async {
    ...
    #expect(store.captureReadiness.detail == "Grant Screen Recording, then reopen OpenCue.")
...
func clipMarkerExporterWritesJSONPayload() throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-clip-export-\(UUID().uuidString)", isDirectory: true)
...
func clipMarkerExporterAvoidsSameSecondOverwrite() throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-clip-export-collision-\(UUID().uuidString)", isDirectory: true)
    let marker = ClipMarker(
        title: "Clip Face",
        reason: "Great explanation.",
        scene: .face,
...
func studioStoreExportsClipMarkers() async {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-store-export-\(UUID().uuidString)", isDirectory: true)
    let store = StudioStore(mediaPipeline: SpyMediaPipeline())
...
func newCaptureSessionClearsPreviousClipArtifacts() async {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-session-clear-\(UUID().uuidString)", isDirectory: true)
    let store = StudioStore(mediaPipeline: SpyMediaPipeline())
...
func sessionReportExporterWritesJSONPayload() throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-session-export-\(UUID().uuidString)", isDirectory: true)
...
func sessionReportExporterAvoidsSameSecondOverwrite() throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-session-export-collision-\(UUID().uuidString)", isDirectory: true)
    let report = SessionReportPayload(
...
func studioStoreExportsSessionReportWithoutSecretURL() async throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-session-store-\(UUID().uuidString)", isDirectory: true)
    let store = StudioStore(mediaPipeline: SpyMediaPipeline())
...
func studioStoreRetainsExtendedCurrentSessionEventsForReport() async throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-session-events-\(UUID().uuidString)", isDirectory: true)
    let store = StudioStore(mediaPipeline: SpyMediaPipeline())
...
func studioStoreExportsSourceAndCaptureTargetContextInSessionReport() async throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("opencue-session-context-\(UUID().uuidString)", isDirectory: true)
    let report = CapturePreflightReport(
        devices: [
            CaptureDeviceInfo(id: "display-7", kind: .display, name: "Studio Display", detail: "3024x1964", permission: .granted),
            CaptureDeviceInfo(id: "window-42", kind: .window, name: "Slides", detail: "Keynote", permission: .granted)
...
private final class ConfigurableMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-configurable.mov")
    }
...
private final class TransportCountingMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-transport-counting.mov")
    }
...
private final class ReadinessGatedMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-readiness-gated.mov")
    }
...
private final class ScreenVideoGatedMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-screen-video-gated.mov")
    }
...
private final class RecoveringMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-recovering.mov")
    }
...
private final class FlakyStartMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-flaky.mov")
    }
...
private final class DelayedStartMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/opencue-delayed.mov")
    }
...
private final class DelayedStopMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-delayed-stop.mov")
    }
...
private final class DelayedStopRecordingPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-delayed-stop-recording.mov")
    }
...
private final class NonCancellableDelayedStartMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        URL(fileURLWithPath: "/tmp/opencue-noncancellable.mov")
    }
...
private final class NonCancellableDelayedRecordingPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/opencue-delayed-recording.mov")
    }
...
private final class SpyMediaPipeline: MediaPipeline, @unchecked Sendable {
    ...
    func startRecording() async throws -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/opencue-test.mov")
    }
