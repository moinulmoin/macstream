...
6. MLX setup inference can be paused or deprioritized under stream pressure.
...
Source level controls should stay coarse enough to avoid churn. OpenCue normalizes source levels to 1% steps and uses the same step in SwiftUI sliders, so tiny pointer/trackpad deltas do not repeatedly reconfigure the signal provider or media pipeline.

Active capture should not allow the selected scene to lose its required visual source or move to a scene the current media path cannot output. Required camera/screen source toggles and required source level controls are locked while streaming, connecting, recording, or stopping, and required level-controlled sources must be nonzero before real capture starts. The setup recovery action must repair both disabled needed sources and enabled-but-zero needed source levels before capture. Real ScreenCaptureKit publishing blocks Face/Screen + Face/BRB scene switches until publish composition exists, and local recording blocks scene switching while the writer is active so the file does not mix incompatible layouts. Optional audio toggles and levels remain available so the operator can still fix sound without forcing a scene mismatch or capture restart.
...
- HaishinKit for RTMP/SRT feasibility. It is an explicit opt-in build path via `OPEN_CUE_ENABLE_HAISHINKIT=1` because the package currently resolves additional binary artifacts beyond the RTMP products OpenCue needs.
- mlx-swift-lm through the explicit `OPEN_CUE_ENABLE_MLX=1` build path after the director/rule schema is stable. Keep the default build dependency-light and treat the model loader/generation step as a separate integration gate.

No Python sidecar in the MVP.

## Local Model Strategy

MLX belongs in setup, summaries, and post-session assistance first. The live director should keep using typed profiles and deterministic thresholds so scene changes do not depend on model latency, token streaming, or model download state.

The preferred setup model is `LiquidAI/LFM2.5-8B-A1B-MLX-4bit` once the MLX path is linked, because it is MLX-native and strong enough for instruction-heavy setup/rule generation. It should not run in the live capture loop.

Setup-rule generation should stay disabled while streaming, connecting, recording, or missing a real stream description so local model work cannot steal CPU, memory bandwidth, or thermal headroom from capture. If capture starts while setup generation is already in flight, OpenCue cancels the task and surfaces a setup-paused warning instead of waiting for a local model to finish. Setup prompts are trimmed and bounded before reaching either the fallback rule engine or the MLX prompt builder so pasted notes cannot create unexpectedly large local-model requests, and results from stale prompt text are discarded before they can change director rules.

Vision models belong in a separate multimodal adapter, not the hot path. Moondream is a good local candidate for sampled frames when OpenCue needs object detection, visual question answering, screen safety checks, or "where is the user's attention" cues. The adapter should convert vision output into bounded typed signals, cache aggressively, and sample slowly enough that camera/screen preview, recording, and streaming stay ahead of inference. Optional cloud vision can support users who want higher quality or do not have local model headroom, but it must be visible in the UI and disabled by default.
