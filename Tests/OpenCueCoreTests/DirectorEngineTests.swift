...
@Test
func buildScriptDefinesRequiredCapturePrivacyKeys() throws {
    ...
    #expect(script.contains("NSAudioCaptureUsageDescription"))
}

@Test
func buildScriptSignsAppBundleWithStableIdentifier() throws {
    let scriptURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("script/build_and_run.sh")
    let script = try String(contentsOf: scriptURL, encoding: .utf8)

    #expect(script.contains("BUNDLE_ID=\"com.ideaplexa.opencue\""))
    #expect(!script.contains("BUNDLE_ID=\"com.opencue.app\""))
    #expect(script.contains("Developer ID Application: Ideaplexa LLC (53P98M92V7)"))
    #expect(script.contains("OPEN_CUE_CODESIGN_IDENTITY"))
    #expect(script.contains("/usr/bin/codesign --force --sign \"$SIGN_IDENTITY\" --identifier \"$BUNDLE_ID\" --timestamp=none \"$APP_BUNDLE\""))
    #expect(script.contains("--requirements \"=designated => identifier \\\"$BUNDLE_ID\\\"\""))
    #expect(script.contains("/usr/bin/codesign --verify --strict \"$APP_BUNDLE\""))
    #expect(script.contains("xattr -cr \"$APP_BUNDLE\""))
    #expect(script.contains("expected code signature identifier $BUNDLE_ID"))
    #expect(script.contains("expected stable designated requirement"))
}
